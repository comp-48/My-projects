...
class VideoRecorderApp:
    ...
    def create_widgets(self):
        ...
        self.camera_info_label = tk.Label(self.root, text="Камера: не подключена", font=("Arial", 9), fg="red")
    ...
    def connect_camera(self, try_indices=None):
        ...
        for idx in set(try_indices):
            ...
            if cap.isOpened():
                ...
                if ret and frame is not None:
                    ...
                    self.camera_info_label.config(text=f"Камера: индекс {idx} (подключена)", fg="green")
                ...
                if api_preference == cv2.CAP_DSHOW:
                    ...
                    if cap2.isOpened():
                        ...
                        if ret and frame is not None:
                            ...
                            self.cap = cap2
                            self.camera_index = idx
                            self.camera_info_label.config(text=f"Камера: индекс {idx} (подключена, без DSHOW)", fg="green")
        ...
        self.camera_info_label.config(text="Камера: НЕ подключена! Проверьте индексы.", fg="red")
        if not hasattr(self, '_error_shown'):
            ...
            messagebox.showerror("Ошибка", 
                ...
                "или выберите индекс вручную в настройках.")
        return False
    ...
    def open_settings(self):
        ...
        tk.Label(settings_win, text="Индекс камеры (0,1,2...):").pack(pady=5)
        cam_idx_var = tk.StringVar(value=str(self.camera_index))
        tk.Entry(settings_win, textvariable=cam_idx_var, width=10).pack(pady=5)
        ...
        def import_settings():
            ...
            try:
                ...
                if "target_fps" in data and data["target_fps"] > 0:
                    ...
                    self.root.after(100, self.reconnect_camera)
                if "camera_index" in data and data["camera_index"] >= 0:
                    self.camera_index = data["camera_index"]
                    cam_idx_var.set(str(self.camera_index))
